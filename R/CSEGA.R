...
CSEGA <- function(seu,
  ...
  if (is.numeric(threshold_method)) {
    ...
    for (t in qs) {
      ...
      if (!is.na(stat) && stat > best_stat) { best_stat <- stat; best_q <- t }
    }
    if (is.na(best_q)) {
  ...
  } else { stop("threshold_method must be numeric, 'median','mean','q0.xx','kmeans', or 'learn'.") }
  ...
  mat <- prop.table(tab, margin = 2)
  storage.mode(mat) <- "numeric"
  
  # ---- logistic regression per celltype ----
  ...
  prop_stats <- lapply(celltypes, function(ct) {
    ...
    tibble::tibble(celltype = ct, x_high = x_high, n_high = n_high, x_low = x_low, n_low = n_low, fold_change = fold_change, odds_ratio = or, p_value = pval, p_high = p_high, p_low = p_low)
  }) %>% dplyr::bind_rows()
  
  final_df <- summary_df %>% left_join(prop_stats, by = "celltype") %>%
    dplyr::mutate(p_adj = ifelse(is.na(p), NA_real_, p.adjust(p, method = "BH")),
                  sig_label = dplyr::case_when(!is.na(p_adj) & p_adj < 0.001 ~ "***",
  ...
  # ---- bubble plot ----
  bubble_df <- prop_stats %>%
    tidyr::pivot_longer(cols = c("p_high", "p_low"), names_to = "expr_group", values_to = "proportion") %>%
    mutate(expr_group = ifelse(expr_group == "p_high", "High", "Low")) %>%
    left_join(final_df %>% select(celltype, beta, sig_label), by = "celltype") %>%
    mutate(beta_for_plot = ifelse(is.na(beta), 0, beta))
  x_max <- mean(as.numeric(factor(bubble_df$expr_group)))-0.05
